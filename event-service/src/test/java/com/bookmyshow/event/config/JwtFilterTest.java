...
class JwtFilterTest {
	...
	void testDoFilterInternal_validToken_shouldSetAuthenticationAndCallChain() throws Exception {
		...
		when(request.getHeader("Authorization")).thenReturn(EventConstants.BEARER_PREFIX + token);
		Claims claims = mock(Claims.class);
		when(claims.getSubject()).thenReturn("user@example.com");
		when(claims.get(EventConstants.CLAIM_USER_ID, Long.class)).thenReturn(10L);
		when(claims.get(EventConstants.CLAIM_ROLES)).thenReturn(List.of("ROLE_USER"));
